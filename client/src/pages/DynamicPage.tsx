...
export default function DynamicPage({
  ...
  const { data: contentData, isLoading: contentLoading } =
    trpc.website.getPageContent.useQuery({
      pageKey,
    });
  ...
  useEffect(() => {
    ...
    if (navigationData?.title) {
      document.title = `${navigationData.title} - 医美`;
    }
  }, [navigationData]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#B8A68D] mx-auto mb-4"></div>
    ...
    <div className="min-h-screen bg-page-soft">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-lg font-bold text-stone-600 font-heading">
                医美
              </span>
            </div>
      ...
      <div className="container mx-auto px-4 py-4">
        <Button
          variant="ghost"
          onClick={() =>
            location === "/" ? setLocation("/") : setLocation(-1 as any)
          }
          className="gap-2"
        >
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {navigationData && (
          <div className="mb-8">
            <h1 className="mb-2 text-stone-700">{navigationData.title}</h1>
        ...
        {Object.entries(groupedContent).map(([sectionKey, items]) => (
          <Card key={sectionKey} className="mb-8 border-stone-200 shadow-sm">
            <CardContent className="pt-6">
              <div className="space-y-4">{items.map(renderContentItem)}</div>
            </CardContent>
          </Card>
        ))}
      ...
      {/* 底部 */}
      <footer className="bg-[#3D3832] text-[#D5CFC7] py-8 mt-20">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold font-heading text-[#FAF9F7]">
                医美
              </span>
            </div>
            <div className="flex items-center gap-6 text-[#A39E97] text-sm">
              <span>服务时间：9:00 - 21:00</span>
              <span>|</span>
              <span>© 2026 医美 版权所有</span>
            </div>
          </div>
        </div>
      </footer>
                  ...
                  <div key={item.id} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setLocation(`/dashboard/website/content/${item.id}`)
                        }
                      >
